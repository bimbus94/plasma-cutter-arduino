...
StepperModel xAxisStepper(
    ...
    XAXIS_STEPS_PER_FULL_ROTATION, XAXIS_MICROSTEPPING, XAXIS_GEAR_RATIO);
...
StepperModel rotationStepper(
    ...
    YAXIS_STEPS_PER_FULL_ROTATION, YAXIS_MICROSTEPPING, YAXIS_GEAR_RATIO);
...
//lcd.println("");
    ...
    //memset(entryStr, 0, sizeof(entryStr));
    //fileName = fileName + ".txt";
    //sscanf(entryStr, "%d", &start_pointX);
...
//lcd.println("");
    ...
    //memset(entryStr, 0, sizeof(entryStr));
    //fileName = fileName + ".txt";
    //sscanf(entryStr, "%d", &start_pointY);
...
//myFile.print(diameter);
// stara wersja, robila okregi, nie zwazajac na rzutowania
/* old cut radius
    if( cut_radius != 0)
    {
        tempX1=start_pointX-2*cut_radius*sin(cut_angle);
        tempY1=start_pointY+2*cut_radius*cos(cut_angle);

        //Serial.println(tempX1);
        tempI1=start_pointX-cut_radius*sin(cut_angle);
        tempJ1=start_pointY+cut_radius*cos(cut_angle);

        tempX3=start_pointX-(M_PI*diameter/2)*tan(cut_angle);
        tempY3=start_pointY+M_PI*diameter/2;

        tempX2=tempX3+2*cut_radius*sin(cut_angle);
        tempY2=tempY3+2*cut_radius*cos(cut_angle);

        tempI2=tempX3+cut_radius*sin(cut_angle);
        tempJ2=tempY3+cut_radius*cos(cut_angle);
...
{		
    ...
    else if(cut_radius == 0)
    {
	   ...
	   tempY3=start_pointY+alpha_y*diameter/2;
		...
		/*
        tempX2=start_pointX;
        tempY2=start_pointY+M_PI*diameter;
        myFile.print("G1X");
        myFile.print(tempX2);
        myFile.print("Y");
        myFile.println(tempY2);
		*/
